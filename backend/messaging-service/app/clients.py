...
def get_messaging_privacy(user_id):
    """
    "everyone" (défaut) ou "nobody" — réglage Paramètres > Confidentialité
    de l'utilisateur ciblé. En cas de doute (service injoignable), on
    n'empêche jamais l'échange : mieux vaut un message de plus qu'un
    utilisateur bloqué à tort par une panne réseau interne.
    """
    try:
        r = requests.get(
            f"{Config.USER_SERVICE_URL}/internal/users/{user_id}",
        ...
        if r.status_code == 200:
            return r.json()["data"].get("messagingPrivacy", "everyone")
    except requests.RequestException as e:
        logger.error("User Service injoignable (confidentialité)", userId=user_id, error=str(e))
    return "everyone"
